...
private[spark] class SparkSubmit extends Logging {
  ...
  def doSubmit(args: Array[String]): Unit = {
    ...
    // action = Option(action).getOrElse(SUBMIT) 默认值为 submit
  ...
  : (Seq[String], Seq[String], SparkConf, String) = {
    ...
    // yarn 模式 childMainClass = org.apache.spark.deploy.yarn.YarnClusterApplication
    if (isYarnCluster) {
      ...
      if (args.isPython) {
        ...
        if (args.primaryResource != SparkLauncher.NO_RESOURCE) {
          // --jar ./examples/jars/spark-examples_2.11-2.1.1.jar
    ...
    // Mesos 模式的 childMainClass = org.apache.spark.deploy.rest.RestSubmissionClientApp
    if (isMesosCluster) {
    ...
    // k8s 集群的 childMainClass = org.apache.spark.deploy.k8s.submit.KubernetesClientApplication
    if (isKubernetesCluster) {
  ...
  private def runMain(args: SparkSubmitArguments, uninitLog: Boolean): Unit = {
    ...
    //    Mesos 模式的 childMainClass = org.apache.spark.deploy.rest.RestSubmissionClientApp
    //    yarn 模式 childMainClass = org.apache.spark.deploy.yarn.YarnClusterApplication
    //    standalone 模式 childMainClass = org.apache.spark.deploy.rest.RestSubmissionClient
    ...
    try {
      ...
      mainClass = Utils.classForName(childMainClass) // YarnClusterApplication
    ...
    val app: SparkApplication = if (classOf[SparkApplication].isAssignableFrom(mainClass)) {
      ...
      // 继承了 SparkApplication 的类通过构造器创建
      mainClass.getConstructor().newInstance().asInstanceOf[SparkApplication]
    } else {
      // 其他的直接 new 一个 SparkApplication
    ...
    try {
      ...
      // 调用 org.apache.spark.deploy.yarn.YarnClusterApplication$start 方法
      // 该类在 [resource-managers]文件下 的 org.apache.spark.deploy.yarn.Client 类中
      // childArgs =
      //    --jar ./examples/jars/spark-examples_2.11-2.1.1.jar
      //    --class org.apache.spark.examples.SparkPi
...
object SparkSubmit extends CommandLineUtils with Logging {
  ...
  底层运行
    ...
    最终底层运行=> bin/java org.apache.spark.deploy.SparkSubmit "$@"

    最终执行 org.apache.spark.deploy.SparkSubmit#main
